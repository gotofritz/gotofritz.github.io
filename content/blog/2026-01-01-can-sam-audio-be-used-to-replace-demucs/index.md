...
How long should these segments be? One Meta developer suggested 10 seconds, since those are the clip lengths the models were trained on. I use values between 9 and 18 seconds for the sam-audio-large model. So a typical 3 to 4 minute track ends up split into roughly 20 segments. **Important**: <nobr>SAM Audio</nobr> pads output files to the closest decimal (<nobr>i.e.</nobr> if you feed it a 12.778980s file, it would generate a 12.8s output)
